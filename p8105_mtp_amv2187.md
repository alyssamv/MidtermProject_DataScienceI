...
Since we also have information on the day of the week (DOW), we can test the hypothesis that DOW can also predict activity level. However, an ANOVA test suggests that there is little evidence to suggest a correlation; that is, that the average activity level varies by DOW (Figure 2a). This holds true when accounting for time (Figure 2b), and it may be noted that time itself remains a significant predictor of activity level. Interestingly, Saturday goes from being, on average, the least active DOW to the most active by the end of the study period.
...
``` r
# smoothed 24-hour trends for each day of the week
#hourly_DOW = 
accel_data %>%
  group_by(week, day, hour) %>% # group by week, day, and hour
  summarise(total_activity_hr = sum(activity_level)) %>% # get total activity for each hour of every day
  ...
  mutate(day_over_time = ceiling(row_number()/24)) %>% # variable to group by day 1:329
  group_by(day) %>%
  ggplot(aes(y = total_activity_hr, x = hour, group = day, 
             color = day)) +
  geom_smooth(se = F) +
  labs(
    title = '24-hour activity profile by DOW',
    x = 'Hour of day (beginning at 12 am)',
    y = 'Activity Level',
    caption = 'Figure 3',
...
accel_data %>%
  ...
  ungroup %>%
  mutate(day_over_time = ceiling(row_number()/24)) %>% # variable to group by day 1:329
  ggplot(aes(x = day_over_time, y = hour)) +
  geom_point(aes(size = total_activity_hr), alpha = 0.2) +
  labs(
    title = 'Hourly activity level over time',
    y = 'Hour of day (beginning at 12 am)',
    x = 'Time (days)',
    #caption = 'Figure 4',
    size = 'Activity level'
  ) + 
